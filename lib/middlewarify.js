var debug = require('debug')('expressx');
var assert = require('assert');
var merge = require('util')._extend;
var mergePhaseNameLists = require('./merge-name-lists');
...
module.exports = function middlewarify(app) {
    app.__expressLazyRouter = app.lazyrouter || app.express.lazyrouter; // support witty and locomotive app
    merge(app, proto);
};
...
proto.middleware = function (name, paths, handler) {
    ...
    // support nested express application
    var fn = handler;
    // non-express app
    if (!handler || !handler.handle || !handler.set) {
        debug('use %s %s %s', fullPhaseName, paths, handlerName);
        this.use(paths, fn);
    } else {
        debug('use app %s %s %s', fullPhaseName, paths, handlerName);
        handler.mountpath = paths;
        handler.parent = this;
    ...
    var layer = this._findLayerByHandler(fn);
    if (layer) {
